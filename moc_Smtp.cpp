/****************************************************************************
** Meta object code from reading C++ file 'Smtp.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.2.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "Smtp.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'Smtp.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.2.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_Smtp_t {
    QByteArrayData data[13];
    char stringdata[109];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    offsetof(qt_meta_stringdata_Smtp_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData) \
    )
static const qt_meta_stringdata_Smtp_t qt_meta_stringdata_Smtp = {
    {
QT_MOC_LITERAL(0, 0, 4),
QT_MOC_LITERAL(1, 5, 9),
QT_MOC_LITERAL(2, 15, 0),
QT_MOC_LITERAL(3, 16, 7),
QT_MOC_LITERAL(4, 24, 8),
QT_MOC_LITERAL(5, 33, 7),
QT_MOC_LITERAL(6, 41, 9),
QT_MOC_LITERAL(7, 51, 1),
QT_MOC_LITERAL(8, 53, 15),
QT_MOC_LITERAL(9, 69, 9),
QT_MOC_LITERAL(10, 79, 12),
QT_MOC_LITERAL(11, 92, 11),
QT_MOC_LITERAL(12, 104, 3)
    },
    "Smtp\0pipeReady\0\0failure\0response\0"
    "success\0printLine\0s\0connectionReady\0"
    "readyRead\0disconnected\0sendCommand\0"
    "cmd\0"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_Smtp[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       8,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       4,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    0,   54,    2, 0x06,
       3,    1,   55,    2, 0x06,
       5,    1,   58,    2, 0x06,
       6,    1,   61,    2, 0x06,

 // slots: name, argc, parameters, tag, flags
       8,    0,   64,    2, 0x08,
       9,    0,   65,    2, 0x08,
      10,    0,   66,    2, 0x08,
      11,    1,   67,    2, 0x08,

 // signals: parameters
    QMetaType::Void,
    QMetaType::Void, QMetaType::QString,    4,
    QMetaType::Void, QMetaType::QString,    4,
    QMetaType::Void, QMetaType::QString,    7,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void, QMetaType::QString,   12,

       0        // eod
};

void Smtp::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Smtp *_t = static_cast<Smtp *>(_o);
        switch (_id) {
        case 0: _t->pipeReady(); break;
        case 1: _t->failure((*reinterpret_cast< QString(*)>(_a[1]))); break;
        case 2: _t->success((*reinterpret_cast< QString(*)>(_a[1]))); break;
        case 3: _t->printLine((*reinterpret_cast< QString(*)>(_a[1]))); break;
        case 4: _t->connectionReady(); break;
        case 5: _t->readyRead(); break;
        case 6: _t->disconnected(); break;
        case 7: _t->sendCommand((*reinterpret_cast< QString(*)>(_a[1]))); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (Smtp::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&Smtp::pipeReady)) {
                *result = 0;
            }
        }
        {
            typedef void (Smtp::*_t)(QString );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&Smtp::failure)) {
                *result = 1;
            }
        }
        {
            typedef void (Smtp::*_t)(QString );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&Smtp::success)) {
                *result = 2;
            }
        }
        {
            typedef void (Smtp::*_t)(QString );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&Smtp::printLine)) {
                *result = 3;
            }
        }
    }
}

const QMetaObject Smtp::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_Smtp.data,
      qt_meta_data_Smtp,  qt_static_metacall, 0, 0}
};


const QMetaObject *Smtp::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *Smtp::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_Smtp.stringdata))
        return static_cast<void*>(const_cast< Smtp*>(this));
    return QObject::qt_metacast(_clname);
}

int Smtp::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 8)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 8;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 8)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 8;
    }
    return _id;
}

// SIGNAL 0
void Smtp::pipeReady()
{
    QMetaObject::activate(this, &staticMetaObject, 0, 0);
}

// SIGNAL 1
void Smtp::failure(QString _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void Smtp::success(QString _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 2, _a);
}

// SIGNAL 3
void Smtp::printLine(QString _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 3, _a);
}
QT_END_MOC_NAMESPACE
